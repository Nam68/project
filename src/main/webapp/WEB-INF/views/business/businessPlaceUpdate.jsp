...
<c:set value="${placeList[0] }" var="place"></c:set>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
    ...
    <script>
	    ...
	    window.onload = function() {
	    	let prelat = ${place.pl_lat};
	    	let prelng = ${place.pl_lng};
	    	reverseGeo(prelat, prelng);
	    }
    </script>
	<!-- 주소 관련 스크립트 (수정 절대X) -->
	<script>
    ...
    <script>
    	let map;
    	function reverseGeo(prelat, prelng) { //지리정보를 주소로 변환
    		var geocoder = new google.maps.Geocoder();
    		var target = new google.maps.LatLng(prelat, prelng);
    		var request = {
    			location:target
    		};
    		geocoder.geocode(request, function(results, status) {
    			let data = JSON.stringify(results[0].formatted_address);
    			data = data.substring(1, data.length-1);
    			document.form.roadAddrPart1.value = data;
    		});
    	}
    	function addrToGeometry(placename) { //주소를 지리정보로 변환
    	...
    	function callbackText(results, status) {
    		if (status == google.maps.places.PlacesServiceStatus.OK) {
    			...
    			sendXHRwithJSON('businessPlaceUpdate.do', data, function() {
    				if(XHR.readyState == 4) {
    					if(XHR.status == 200) {
	...
	<!-- 리뷰 관련 -->
	<script>
		function reviewDeleteReq(r_idx) {
			window.alert('정당하지 않은 삭제 요청은 반려될 수 있습니다');
			let params = 'r_idx='+r_idx;
			sendXHR('reviewDeleteReq.do', params, reviewDeleteReqResult, 'GET');
		}
		function reviewDeleteReqResult() {
			if(XHR.readyState == 4) {
				if(XHR.status == 200) {
					window.alert(XHR.responseText);
				}
...
<body>
	<form name="form" id="form" method="post">
		...
		<div>
			<div>사업장명</div>
			<div>
				<input  class="placeText" type="text" id="businessPlaceName" value="${place.pl_name }">
			</div>
		</div>
		<!-- 주소AIP (수정 절대X) -->
		<div>
					...
					<c:if test="${status.index%3==0 }"><tr></c:if>
						<td id="favoriteChkTd">
							<span class="favoriteChk">
								<label>
								<input type="checkbox" name="favorite" value="${f.v_idx }" onclick="checkboxCheck()" ${f.count>0? 'checked':'' }>${f.v_name }
								</label>
							</span>
						</td>
					...
					<tr>
						<td><div class="imgDiv">
							...
							<c:if test="${!empty placeList[0] }">
								...
								<img alt="exampleImg" src="${placeList[0].pl_img }" class="businessExampleImg" onclick="imgFileOpen(0)">
							</c:if>
						</div></td>
						<td><div class="imgDiv">
							<c:if test="${empty placeList[1] }">
							...
							<c:if test="${!empty placeList[1] }">
								...
								<img alt="exampleImg" src="${placeList[1].pl_img }" class="businessExampleImg" onclick="imgFileOpen(1)">
							</c:if>
						</div></td>
					</tr>
					<tr>
						<td><div class="imgDiv">
							<c:if test="${empty placeList[2] }">
							...
							<c:if test="${!empty placeList[2] }">
								...
								<img alt="exampleImg" src="${placeList[2].pl_img }" class="businessExampleImg" onclick="imgFileOpen(2)">
							</c:if>
						</div></td>
						<td><div class="imgDiv">
							<c:if test="${empty placeList[3] }">
							...
							<c:if test="${!empty placeList[3] }">
								...
								<img alt="exampleImg" src="${placeList[3].pl_img }" class="businessExampleImg" onclick="imgFileOpen(3)">
							</c:if>
						</div></td>
					</tr>
				</tbody>
				...
				<tbody>
					<tr>
						<td><textarea id="businessPlaceContent" onfocus="textAreaClear(this)"
								cols="60" rows="10">${place.pl_content=''? '우리 가게만의 매력을 전해보세요':place.pl_content }</textarea></td>
					</tr>
				</tbody>
			</table>
		</div>
	...
	<div>
		<input type="button" value="등록" onclick="businessPlaceUpdate()">
	...
	<div>
		...
		<c:if test="${!empty reviewList }">
		<div>
			<div class="starAvg">평균 : ${place.pl_star }점</div>
			<div class="reviewBody">
				<c:forEach var="r" items="${reviewList }">
					<div class="reviewContent">
						<div class="reviewReview">${r.review }</div>
						<div class="reivewStar">${r.star }점</div>
						<div class="reviewDelete"><input type="button" value="삭제요청" onclick="reviewDeleteReq(${r.r_idx})"></div>
					</div>
				</c:forEach>
			</div>
		</div>
