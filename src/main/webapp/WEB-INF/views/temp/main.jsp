...
<head>
    ...
    <style>
        ...
        .aside{width: 20%; float: left;  height: calc( 100% - 250px); overflow: auto; }
        .placeSearchBody{background-color: white;}
        
        .footer{width: 100%; border-top:5px solid #7fc9cb; height: 150px; overflow: hidden; margin:0 auto; position: relative;}
        .dayButton{position: absolute; left: 0; top: 0;;}
        .cartDay{background-color: #7fc9cb; padding: 3%;}
        .cartDay:hover{cursor: pointer;}
        
        li{list-style-type: none;}
        
        .memberJoinButton:link{text-decoration: none; color: black;}
        .memberJoinButton:visited{text-decoration: none; color: black;}
        .memberLoginButton:hover{cursor: pointer;}
        ...
        /* 검색창 관련 CSS */
        .placeSearch{position: relative;}
        .placeSearchResult {width: 13.8vw; height:23vh; position: absolute; top:4vh; left: 3.3vw; background-color: white; display: none;}
        .searchResultText:hover {background-color: #AAA;}
        #placeSearchTextfield {width: 70%; margin-left: 0.2vw; margin-top: 10px; border-radius: 4px;}
        .placeSearchIcon{margin-left: 1vw; }
        #searchResultClose:hover{background-color: red; cursor: pointer;}
		...
		.cartDurationDiv{vertical-align: middle;}
        ...
        /* 로그인 관련 */
        #loginDiv {float: right; vertical-align: middle; margin: 3%;}
        .loginBody>div {text-align: center; margin-bottom: 5%;}
        .loginTitle {font-size: 3vw;}
        .loginBody>div>div {display: inline-block; text-align: center; margin-left: 1%; margin-right: 1%;}
	    ...
	    function loginSubmitResult() {
	    	if(XHR.readyState == 4) {
	    		if(XHR.status == 200) {
	    			...
	    			if(data.groupidx == 3) {
	    				window.onbeforeunload = function(){sendXHR('saveFavorite.do', null, null, 'GET');};
	    				location.href='businessPage.do';
	    			} else if(data.groupidx == 1) {
	    				window.onbeforeunload = function(){sendXHR('saveFavorite.do', null, null, 'GET');};
	    				location.href='customerForm.do';
	    			}
	    			
	    			window.alert(data.msg);
	    			loginDiv.innerHTML = data.id+'님 로그인 중 | <a onclick="logoutSubmit()">로그아웃</a>';
    ...
    <script>
    	...
    	function searchResultShow(data) { //가져온 결과를 사용자에게 보여주는 메서드
    		...
    		searchText += '<div class="searchResultClose" onclick="searchResultClose()">닫기</div>';
    		searchResultWindow.innerHTML = searchText;
    		
    	}
    	function searchForClick(pl_idx) { //검색 결과를 클릭하면 발생
    		let params = 'pl_idx='+pl_idx;
    		sendXHR('placeSearchIdx.do', params, searchForClickResult, 'GET');
    	}
    	function searchForClickResult() {
    		if(XHR.readyState == 4) {
    			if(XHR.status == 200) {
    				placeSearchBody.innerHTML = XHR.responseText;
    	...
    	function searchResultClose() { //검색창 닫음
    		const resultWindow = document.getElementsByClassName('placeSearchResult')[0];
			...
			if(XHR.readyState == 4) {
				if(XHR.status == 200) {
					...
					for(var i = 0; i < cartData.length; i++) {
						...
						if(mapCost[i] != null) {
							let tempDuration = 
							'<span class="cartDurationDiv">'+
								Math.round(mapCost[i].duration/240)+'분'+
							'</span>';
							tempText += tempDuration;
						}
						cartDiv += tempText;	
					}
...
<body>
    <div class="wrap">
        <div class="header">
            ...
            <div id="loginDiv">
            	<c:if test="${!empty sessionScope.memberDTO }">${sessionScope.memberDTO.id }님 로그인 중 | <a onclick="logoutSubmit()">로그아웃</a></c:if>
            	<c:if test="${empty sessionScope.memberDTO }"><a class="memberLoginButton" onclick="loginOn()">로그인</a> | <a class="memberJoinButton" href="memberType.do">회원가입</a></c:if>
            </div>
        </div>
        <div class="section" id="map"></div>
	        <div class="aside">
	        	...
	        	<div class="placeSearch">
	        		<span class="placeSearchIcon">검색</span>
	        		<input type="text" id="placeSearchTextfield" onkeyup="onSearchResultOn(this)">
	        		<div class="placeSearchResult">검색창</div>
	        	</div>
	        	<div id="placeSearchBody">
	        	...
	        	<div class="placeInfo">
	        		<span class="placeInfoCloseButton">▶</span>
	        		<div>
		        		<div class="placeInfoTitle">장소 이름</div>
		        		<div class="placeInfoImg">
		        			<div><img class="placeImgs" alt="place" src="#"></div>
		        			<div class="placeInfoImgPageButton"></div>
		        		</div>
		        		<div class="placeInfoContent">
		        			장소 설명 입력창
		        		</div>
		        		<div class="placeInfoAppraisal">
		        			<div class="placeInfoStar">
		        				<div>별</div><div>별점</div>
		        			</div>
		        			<div class="placeInfoLike">
		        				<div class="placeInfoLikeBtn">손가락</div><div>좋아요 수</div>
		        			</div>
		        			<div class="placeInfoReview">리뷰 입력창</div>
		        			<div class="placeInfoReviewWrite" onclick="reviewModalOn()">리뷰 더 보기 / 작성</div>
		        		</div>
		        		<div>
		        			<div class="placeInfoCart" onclick="placeCartInput()">장바구니 담기</div>
		        		</div>
	        		</div>
	        	</div>
	        </div>
 		...
 		<div class="footer">
 			<div class="dayButton"></div>
        	<div class="daylibtn one">
	        	<form name="dayForm">
				<ul>
					...
					<li><input type="text" name="lastday" class="datePicker"
						id="endDate" placeholder="종료일자를 입력해주세요" value=""/></li>
					<li><input type="button" class="cartbtn" id="dayOk"value="확인" onclick="daySubmit()"></li>
					<li><input type="button" class="cartbtn" id="short" value="최단거리 최적화"  onclick="calculateDistance()"></li>
				</ul>
				</form>
				</div>
				<div class="cart-day">
				...
				<div class="daylibtn two">
					<ul>
						<li><button href="#" class="cartbtn" id="daySave" data-popup-open="example">저장하기</button></li>
						<li><input type="submit" name="reset" class="cartbtn" id="dayReset" value="초기화"></li>
					</ul>
       		 </div>
        </div>
        ...
        <!-- save-popup -->
		<main>
			<div class="popup" data-popup="example" id="savePopup">
					...
					<div class="popup-contents">
						...
						<form name="planSave">
							<table>
								<tr>
									<th>TITLE</th>
									<td><input type="text" id="cartP_title" name="p_name" value=""></td>
								</tr>
								<tr>
									<th>MEMO</th>
								...
								<tr>
									<td colspan="2" align="center"><input type="button" onclick="planSaveSubmit()"
